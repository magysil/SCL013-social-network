...
export default () => {
  const views = /*html*/`<nav class='nav-flex'>
  <a class='logo' id='home' href='#'><img id = 'logo-oldbeat' src='img/logo.png' alt=''></a>
  <ul class='navigation'>
      <li>
          <div><a href='#/userProfile'><img class='imgPerfilUser' src=''></a></div>
      </li>
  ...
  <div class='bodyprofile'>
      ...
      <div class='profilepic'>
          <img id='profilepicuser' src='img/profile-picture.png'>
      </div>
  ...
  const timeline = divElement.querySelector('#minipost');
  auth.onAuthStateChanged((user) => {
    if (user) {
      ...
      db.collection('users')
        ...
        .get()
        .then(() => {
          const picture = document.querySelector('#profilepicuser');
          console.log('Document data:', user.displayName);
    ...
    db.collection('publicaciones')
      ...
      .onSnapshot((querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
          ...
          const imgPerfil = divElement.querySelector('.imgPerfilUser');
          imgPerfil.setAttribute('src', `${auth.currentUser.photoURL}`);
        });
      });
  });
  // ------------------------------------------------------------------------------------
  // Crea elementos de Usuario Logueado
  // ------------------------------------------------------------------------------------
  const render = (doc) => {
